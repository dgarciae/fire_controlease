...
export function LeaseCard({ lease, isMarked, onPress, deleteLease }: LeaseCardProps) {
  ...
  const { get_next_date, getTotalOccupancyCost } = useMaths(lease);

  /* Const */

  const closestEvent = useMemo(get_next_date, [
  ...
  const totalOccupancyCost = useMemo(getTotalOccupancyCost, [lease.contract.financial]);
