...
export function useMaths(lease: Lease) {
  /* Functions */

  const getTotalOccupancyCost = () => {
    const rent =
  ...
  const get_next_date = () => {
    ...
    const dateDiff = {
      ...
      contractBO: DATES.contractBO
        ? new Date(DATES.contractBO).getTime() - now
        : Infinity,
      externalBO: DATES.externalBO
        ? new Date(DATES.externalBO).getTime() - now
        : Infinity,
  ...
  return { getTotalOccupancyCost, get_next_date };
}
