...
import { type UserSessionType } from "../domain";
import {
  AuthError,
  ControlledError,
...
const _setHttpHeaders = ({ isProtected, multipart }: SetHeaderParams) => {
  const loggedSession = useAuthStore.getState().userSession;
  const headers = new Headers();

  if (!multipart) headers.set("Content-Type", "application/json");
  if (isProtected) headers.set("Authorization", `Bearer ${loggedSession.accessToken}`);
...
export async function httpRequest({
  ...
  try {
    ...
    if (error instanceof TypeError) {
      toast.error("Network error: please check your connection.", TOAST_OPTIONS);
      throw error;
    }
    ...
    if (error instanceof AuthError) {
      ...
      toast.error(`${error.type}: ${error.message}`, TOAST_OPTIONS);
      throw error;
    }
